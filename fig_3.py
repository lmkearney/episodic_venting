...
def p_m_comp(x, t, G, K):
    zeros = roots(K, N)
    p = t + (G - 1.0) * (
        (2 * t - x * (2.0 - x)) / (2.0 * (K + 1.0)) + K / (3.0 * (1.0 + K) ** 2)
    )
    for i in range(1, N):
        p -= (
            2.0
...
for i in range(3):
    ...
    for j in range(len(t)):
        ...
        ax[i][0].plot(
            np.zeros(len(x_2)) + p_m_comp(0, t[j], G, Ks[i]),
            x_2,
            c=cmap(j / (float(len(t)))),
            lw=1.5,
        )
...
ax[0][1].set_ylabel(r"average overpressure $\overline{p^*}$")
